...
### Status Indicators — 平静的状态色(The Calm-Status Rule)
承接 PRODUCT.md 原则 5(状态透明,语气平静):系统状态用**稀缺色 + 小圆点**呈现,
不用红色警报轰炸、不加投影、不铺色块。统一词汇:`h-1.5 w-1.5 rounded-full` 状态点
(色随语义),静止失败点**不闪烁**(闪烁=警报),仅"进行中"用 `animate-pulse`
(配 `motion-reduce:animate-none`)。色义恒定(同色同义,贯穿全站):
- **`--accent`(苔绿)= 在场 / 完成可读**:`TranscriptBadge` 的 `present`(字幕已入,正文完整)。
  这是 ≤10% 苔色被允许做状态点的位置之一。
- **`--muted` = 进行中 / 软诊断**:`pending`(转写中,点带 pulse);banner 的 `orphans`
  (任务疑似丢失,非硬失败)。
- **`--faint` = 缺席但正常**:`unavailable`(无字幕是降级常态,design §2,**绝不**用 `--err`/红)。
- **`--err` = 真实失败**:banner 的 `failingSources` / `failedItems`;沿用 `ContextRail`
  failed-count 既有先例(`text-err font-medium`,其余中性)。

**Status Badge**(`TranscriptBadge`):承 Chip 形(`rounded-full`、`--line` 描边、`text-xs`),
左侧加状态点 + 文字色按上表;仅 video 条目展示,`na`/空 不渲染。

**Status Banner**(`PipelineHealthBanner`):平静通知条,中性 `--surface` 底 + `--line` 描边 +
`rounded-md`,左侧状态点(`--err` 失败 / `--muted` 软诊断)+ 文案 + `--accent` CTA 链接;
`role="status"`(polite,不抢占);健康时不渲染。优先级 failingSources > failedItems > orphans。
