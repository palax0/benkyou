...
- **视频无字幕**：先取 video metadata 拿到 `video_duration`，写入 item。转写决策（给定 duration + 来源类型 + 部署模式 → skip / confirm / transcribe）是 **core 的一个纯函数**（`pipeline/transcribe-policy.ts`）——the transcribe policy runs **only in `extract`** (async paste — the web tier only creates the item; the worker probes + decides). `transcribePolicy` has a single caller.
  ...
  - 手动粘贴 + `duration > video_manual_limit` → over-`video_manual_limit` paste reuses `transcript_status='skipped_too_long'` and **continues** on title/metadata. Rationale: async paste + ffmpeg-worker-only means duration is known only in `extract`; a synchronous reject would force ffprobe into the web tier.
...
- **⚠️ 次序修正（2026-06-22）**：上面"视频无字幕 → transcribePolicy → transcribe"的链路对 **scrape 源（YouTube/Bilibili）当前并不成立**，且 M2b 并未让它成立。实测根因(systematic-debugging)：(1) YouTube item 永不带 `media_url`，`extract.ts:isTranscribeEligible` 以 `media_url != null` 为闸,故 transcribePolicy 对 YouTube 从不触发——M2b 的 Whisper 只覆盖 direct-media 粘贴 + 播客 enclosure；(2) 更根本的是,挡住字幕抓取的反爬墙(无 PoToken)**同时**扣下音频流 URL——字幕与音频是同一失败域,Whisper **不是**反爬的兜底。**真正的共同前置是 PoToken/凭证模型**(captions 和 Whisper 都依赖它),次序应排在"无字幕转写"之前。详见 `specs/2026-06-22-scrape-source-credentials-design.md`。
- **⚠️ 再次修正（2026-06-25）—— PoToken 模型对 YouTube 已废弃**：2026-06-22 设计的 §7 load-bearing spike 于 2026-06-25 实测**证伪**:即便带有效 PoToken,`get_transcript` 仍返回 400 `FAILED_PRECONDITION`(端点级封锁,与 token 无关,上游 `LuanRT/YouTube.js#1102`),音频走 SABR、无 URL 可 `decipher`。字幕与音频确属同一失败域,但**不是** PoToken 能解。**YouTube 的 metadata/字幕/音频已整体迁移到 worker 内的 yt-dlp 子进程**:youtubei.js 对 YouTube 退休;字幕走 yt-dlp `json3`(接受 auto-caption 为 `present`);无字幕视频由 yt-dlp 下载 `bestaudio` 字节后交给**现有** chunked Whisper 链——上面"resolve 一个新鲜 audio URL 交给 Whisper"的模型在 SABR 下不成立,已被字节下载取代。详见 `specs/2026-06-25-youtube-ytdlp-backend-design.md`(**supersedes 2026-06-22 §2/§3/§4/§7 for YouTube**)。Bilibili 仍走 SESSDATA,不受影响。
...
| M2b · 无字幕转写 | 视频转写（chunked + diarization 解析；状态机归属见 §6.2，serverless 边界见 §11.2）/ runner 延迟推进 seam（transcribe 为唯一 consumer 与验证）/ transcribe 队列双 dispatcher 接线 + 面板可见 / transcribe-policy 的 transcribe/confirm/`skipped_serverless` 分支 / 粘贴的"确认转写 + 预估时长"子流程。**前置**：ai_usage 扩展（conversation_id / kind='transcription' / duration_seconds）+ 记账收口到 `core/ai` 封装层（删调用点埋点防双计，§5.3）。**M2b 决议**：seam 形状 → 泛型 `StageOutcome.advance`（无 transcribe guard hole）；重试计数器归属 → pg-boss `retryLimit=2`，`items.attempts` 不混用；`skipped_serverless` 表达 → policy branch 1；job 超时模型 → per-job `transcribeBudgetSec(durationSec)`；`current_stage` 归属 → 整个 transcribe/confirm 窗口保持 `'extract'`。 **范围澄清(2026-06-22)**：M2b 的 Whisper 仅覆盖带 `media_url` 的 item(direct-media 粘贴 + 播客 enclosure)；scrape 源(YouTube/Bilibili)无字幕→Whisper 未在 M2b 接通,且其前置是 PoToken/凭证模型(见 §6.2 次序修正 + `specs/2026-06-22-scrape-source-credentials-design.md`)。 **更新(2026-06-25)**:该 PoToken 前置对 YouTube 已被推翻——2026-06-22 §7 spike 证伪(get_transcript 端点级封锁 + 音频 SABR),YouTube metadata/字幕/音频整体迁移到 yt-dlp 子进程,见 `specs/2026-06-25-youtube-ytdlp-backend-design.md`。 |
